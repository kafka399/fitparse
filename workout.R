...
workout<-read.table('workout.csv',sep=';',header=T,stringsAsFactors=F)
...
pace=data.frame(m=round(workout$distance-head(c(0,workout$distance),-1)),
                s=workout$timestamp/(workout$distance-head(c(0,workout$distance),-1)))
...
paste(floor(mean(laps$value)),':',round((mean(laps$value)-floor(mean(laps$value)))*60),sep='')
